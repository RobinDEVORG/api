package de.robindev.easymcapi.external.jofkos;
...
public class GameProfileFetcher {
	 ...
	 *            The name
	 * @param skinUrl
	 *            Url from the skin image
	 * @param capeUrl
	 *            Url from the cape image
	 * @return A GameProfile built from the arguments
	 * @see GameProfile
	 */
	public static GameProfile getProfile(UUID uuid, String name, String skinUrl, String capeUrl) {
		...
		profile.getProperties().put("textures", new Property("textures", Base64Coder
				.encodeString(String.format(cape ? JSON_CAPE : JSON_SKIN, args.toArray(new Object[args.size()])))));
		return profile;
	}

	/**
	 * Sets the time as long as you want to keep the gameprofiles in cache (-1 =
	 * never remove it)
	 * 
	 * @param time
	 *            cache time (default = -1)
	 */
